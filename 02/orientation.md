# 第2週：データベース設計（1）ER図の読み書き

## 今日のゴール

アプリで扱うデータを表に分け、ER図を見て「どのテーブルに何が入り、どうつながるか」を説明できるようになる。

---

## 前回のおさらい

前回は、<ruby>scaffold<rt>スキャフォールド</rt></ruby>を使って `Article` のアプリを動かしました。

```bash
rails generate scaffold Article title:string body:text
```

このときRailsは、記事を保存するための `articles` テーブルを作っていました。

1つのテーブルだけでもアプリは動きます。ただし、アプリにコメント機能やカテゴリ機能を足そうとすると、1つのテーブルだけでは足りなくなります。

アプリケーションは必ずデータを持ちます。どんなデータを、どんな形で保存するか。この設計がアプリ全体の土台になります。

---

## なぜER図から始めるのか

画面やコードを書く前に、「何を保存するか」を決める必要があります。

たとえばブログアプリなら、最低でも次のようなことを考えます。

- 記事を保存したい
- コメントを保存したい
- どのコメントがどの記事についたものか知りたい

この部分が曖昧なままコードを書き始めると、後でテーブルもコードも崩れます。

だから今週は、コードを書く前に、データの設計を先に固めます。

---

## データベース設計の基本

データベースでは、データを表の形で管理します。

- `テーブル`：同じ種類のデータをまとめる表
- `カラム`：表の項目
- `レコード`：1件分のデータ

たとえば `articles` テーブルは、次のようなイメージです。

| id | title | body |
|---|---|---|
| 1 | はじめてのRails | scaffoldを使うと最初の形がすぐできる... |
| 2 | MVCとは何か | Railsの基本構造を順番に整理して学ぶ... |

この表では：

- `articles` がテーブル名
- `id` `title` `body` がカラム
- 1行ごとが1件のレコード

Railsでは、テーブル名はふつう複数形でつけます。1件の記事は `Article` ですが、記事をまとめて保存するテーブルは `articles` です。

---

## 主キー `id`

各レコードを区別するために、ふつうは `id` を使います。

`id` は、その行を特定するための番号です。同じテーブルの中で、同じ `id` は重複しません。

たとえば：

- `articles` テーブルの `id = 1` は「1番の記事」
- `articles` テーブルの `id = 2` は「2番の記事」

このように、1件を区別するための中心になるカラムを `主キー` と呼びます。

---

## 外部キー

別のテーブルとつなぐために使うのが `外部キー` です。

たとえば、コメント機能を考えます。

- 1つの記事に、複数のコメントがつく
- 各コメントは、どの記事についたかを覚えておく必要がある

このとき `comments` テーブルに `article_id` を持たせます。

| id | article_id | author_name | body |
|---|---|---|---|
| 1 | 1 | 田中 | わかりやすいです |
| 2 | 1 | 鈴木 | 続きも読みたいです |
| 3 | 2 | 佐藤 | MVCの説明が助かりました |

`article_id = 1` なら、「`articles` テーブルの `id = 1` の記事についたコメント」という意味です。

Railsでは、外部キーはふつう `関連先のテーブル名の単数形_id` の形で書きます。

- `article_id`
- `category_id`
- `user_id`

---

## ER図とは

<ruby>ER図<rt>イーアールず</rt></ruby>（Entity-Relationship Diagram）は、テーブル同士の関係を図で表したものです。日本語では「実体関連図」とも呼ばれます。「実体（Entity）」はデータのまとまり（例：記事、コメント）、「関連（Relationship）」はそのつながり（例：記事にコメントがつく）を意味します。

- どんなテーブルがあるか
- 各テーブルにどんなカラムがあるか
- テーブル同士がどうつながるか

を、一目でわかるようにします。

補足：ここに表示しているER図は画像ではなく、Markdownのコードブロックに `mermaid` と書いて表示しています。演習では後でMarkdownの書き方も扱います。授業では `Code` ボタンを押して中身も少し見ます。

今日の題材である「記事とコメント」は、次のように表せます。

```mermaid
erDiagram
    direction LR
    ARTICLES ||--o{ COMMENTS : has

    ARTICLES {
        bigint id PK
        string title
        text body
        datetime created_at
        datetime updated_at
    }

    COMMENTS {
        bigint id PK
        bigint article_id FK
        string author_name
        text body
        datetime created_at
        datetime updated_at
    }
```

### 図の中の記号

カラムの横に書かれている `PK` と `FK` は、それぞれ次の意味です。

| 記号 | 意味 | 例 |
|---|---|---|
| `PK` | <ruby>Primary Key<rt>プライマリキー</rt></ruby>（主キー）。そのテーブルの中で1件を区別するカラム | `id` |
| `FK` | <ruby>Foreign Key<rt>フォーリンキー</rt></ruby>（外部キー）。別のテーブルとつなぐためのカラム | `article_id` |

### 線の両端の記号

テーブルをつなぐ線の両端には、「いくつ関係するか」を表す記号がついています。

| 記号 | 意味 |
|---|---|
| `\|\|` | 必ず1つ |
| `o\|` | 0または1つ |
| `\|{` | 1つ以上 |
| `o{` | 0個以上 |

今回の図を例に読み方を確認します。

```mermaid
erDiagram
    direction LR
    ARTICLES ||--o{ COMMENTS : has
```

- 左側 `||`（ARTICLES側）：必ず1つの記事
- 右側 `o{`（COMMENTS側）：0個以上のコメント
- つまり「1つの記事に、0個以上のコメントがつく」

---

## このER図の読み方

この図から、次のことがわかります。

- `articles` と `comments` という2つのテーブルがある
- 1つの記事に、複数のコメントがつく
- 1つのコメントは、1つの記事に属する
- そのつながりを作るために、`comments` テーブルに `article_id` がある

大事なのは、`外部キーは「多い側」に置く` ことです。

今回なら：

- 記事は1件に対してコメントが複数つく
- だから `article_id` は `comments` 側に置く

---

## 今週から来週、再来週へ

今週は、ER図を読んで書くところまでやります。

- 今週：ER図で「何を保存するか」を整理する
- 来週：そのER図をもとに、マイグレーションを手で書く
- 再来週：モデルに `has_many` と `belongs_to` を書いてつなぐ

つまり、今週の設計が、そのまま次の2週間の土台になります。

ER図の書き方をもう少し別の説明でも見たい人は、次も参考になります。

- [若手プログラマー必読！５分で理解できるER図の書き方５ステップ](https://www.ntt.com/business/services/rink/knowledge/archive_58.html)

---

## まとめ

今日やったこと：

1. テーブル、カラム、レコードの役割を確認した
2. `id` が主キー、`article_id` が外部キーになることを知った
3. ER図でテーブル同士の関係を表せることを知った
4. 今週の設計が、来週のマイグレーションと再来週のアソシエーションにつながることを確認した

> [!IMPORTANT]
> - テーブルは「同じ種類のデータをまとめる表」
> - 外部キーは、別のテーブルとつなぐためのカラム
> - 1対多の関係では、外部キーは「多い側」に置く