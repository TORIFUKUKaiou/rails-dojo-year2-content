...
## Step 36：記事を作成して表示を観察する

Railsコンソールから、新しい記事を1件作成します。
...
```ruby
Article.create(title: "ALBから作成した記事", body: "2台のEC2で表示を確認します")
```

記事一覧で、ブラウザを何度もリロードします。
...
## Step 37：別の記事を作成して変化を観察する

Step 36で接続してEC2インスタンスとは別のEC2インスタンスに接続をして、Railsコンソールから記事を追加します。
...
## Step 38：表示が変わる理由を考える（考察問題・実行しない）
...
## Step 40：高可用性とデータについて考える（考察問題・実行しない）
