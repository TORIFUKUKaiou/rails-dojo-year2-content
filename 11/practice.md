...
3. 画面下部の `ターゲットを確認` に、 `rails-dojo-week11` インスタンスが追加されていることを確認します。
    ...
    ![](../images/tg-register-pending-target.png)
...
3. EC2を複数のAvailability Zoneに置けば、「Railsアプリを稼働し続けること」と「どのEC2からでも同じ記事を読めること」の両方を解決できるでしょうか。
...
3. EC2を複数のAvailability Zoneに置くだけでは、両方は解決できません。EC2を増やすと、片方が止まっても別のEC2でRailsアプリを稼働し続けやすくなります。一方で、今回の記事データはEC2ごとのSQLiteに保存されています。そのため、どのEC2からでも同じ記事を読めるようにするには、2台のRailsアプリが共通のデータベースを使う構成が別に必要です。
