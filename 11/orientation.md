...
- 異なるAvailability ZoneにEC2を置く理由を説明できる
...
## 6. Availability Zoneと高可用性

Availability Zoneは、AWSのリージョン内にある、互いに分離された場所です。
...
flowchart LR
  U["利用者"] --> ALB["ALB"]
  ALB --> EC21["Availability Zone 1<br>EC2 1"]
  ALB --> EC22["Availability Zone 2<br>EC2 2"]
```

片方のRails serverが停止しても、ALBはヘルスチェックに成功しているEC2へ通信を送ります。

一部に障害が起きてもサービスを続けられるようにする考え方を、高可用性といいます。
