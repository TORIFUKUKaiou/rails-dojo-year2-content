...
- Ubuntu 24.04 LTSのEC2インスタンスを作成する
...
授業で指定されたインスタンスタイプを選択します。

特に指示がない場合は、画面で選べる小さなインスタンスタイプを選びます。

### キーペア

今回はSSHではなくSession Managerで接続します。

キーペアは、授業で指定された設定に従ってください。
...
`IAM インスタンスプロファイル`または`IAM ロール`の項目がある場合は、授業で指定されたロールを選択します。
...
Session Managerで接続した直後は、Rails作業に使う `ubuntu` ユーザーではない場合があります。
...
`Ubuntu` や `24.04` という文字が表示されれば、Ubuntu 24.04 LTSのEC2に入っています。
...
終わったら、Railsが使えるか確認します。

```bash
rails -v
```

`Rails` とバージョン番号が表示されれば成功です。
...
> [!NOTE]
> `rails db:prepare` は、必要に応じてデータベース作成やmigrationを行うコマンドです。
> 既に準備済みの場合は、何も大きな変化がないこともあります。
...
画面に何も記事がない場合は、`New article` などのリンクから記事を作成してみます。
...
- [ ] Ubuntu 24.04 LTSのEC2インスタンスを作成できた
