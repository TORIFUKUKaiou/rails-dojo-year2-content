...
ここまでで、壊れたアプリを復旧する側を経験しました。今度は、復旧したアプリに小さな故障を1つだけ入れる出題者と、その症状から原因を探して直す担当に分かれます。

まず、復旧済みの CodeShelf で出題確認用の記事を1件作成してください。

タイトル：

```text
デバッグ問題：CodeShelf の動きを確認する
```

本文：

```text
画面の動きとコードを見比べて、壊れている場所を探します。
```

記事の詳細画面が表示できたら、次の手順で進めます。

1. 出題者を1人決めます。残りの人は、出題者が修正している間はコードを見ません。
2. 出題者は、下の故障カードから1つだけ選び、指定された1箇所だけを変更します。
3. 出題者は、選んだカード名や変更ファイルを、解答担当に伝えません。
4. 解答担当はブラウザで症状を確認し、「どの操作で、何が不自然か」「どのファイルを見るか」を声に出してから修正します。
5. 修正後、ブラウザで正しい動きに戻ったことを確認します。
6. 役割を交代し、別の故障カードでもう一度行います。

> [!IMPORTANT]
> 故障は必ず1回につき1つだけ入れてください。
> `app/assets/stylesheets/`、`db/`、`config/routes.rb`、`.devcontainer/` は変更しません。
> 記事が削除された場合は、次の出題を始める前に、出題確認用の記事をもう一度作成してください。

<details>
<summary>出題者だけが開く：故障カード</summary>

次のうち、1つだけを選んで変更してください。解答担当には、どれを選んだか見せません。

| カード | 対象ファイル | 入れる故障 | 解答担当が確認する症状 |
|---|---|---|---|
| A：編集への入口 | `app/views/articles/show.html.erb` | 「編集する」の行き先を `articles_path` に変更する | 「編集する」を押しても一覧へ戻ってしまう |
| B：本文の表示 | `app/views/articles/show.html.erb` | `simple_format(@article.body)` の `body` を `title` に変更する | 詳細画面で本文の代わりにタイトルが表示される |
| C：本文の更新 | `app/controllers/articles_controller.rb` | `@article.update(article_params)` を `@article.update(title: article_params[:title])` に変更する | 本文を編集しても保存されない |
| D：削除処理 | `app/controllers/articles_controller.rb` | `@article.destroy!` の行を削除する | 削除したはずの記事が一覧に残る |

</details>

同じ進み具合の別チームがいる場合は、別チームが入れた故障を直す対戦にしても構いません。その場合も、故障は1つだけにし、相手へ故障カードや変更ファイルを伝えずに始めてください。

故障カードをすべて試したら、自分たちで別の故障を1つ考えて出題してみましょう。ブラウザで症状を確認でき、原因を説明して元に戻せる故障にしてください。引き続き、`app/assets/stylesheets/`、`db/`、`config/routes.rb`、`.devcontainer/` は変更しません。

最後に、チームで次を説明してください。

1. どの画面操作で症状を確認したか
2. どのファイルを確認し、なぜそこが原因だと判断したか
3. 修正後に、何を操作して直ったと確認したか
