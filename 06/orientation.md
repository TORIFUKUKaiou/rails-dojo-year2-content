...
今日の練習は、2〜3人のチームで取り組みます。

1台の画面を見ながら、コードを入力する人と、エラーを読んで考えを説明する人に役割を分けます。この進め方を、ペアプログラミングと呼びます。

| 役割 | すること |
|---|---|
| ドライバ | キーボードとマウスを操作し、確認や修正を実行する |
| ナビゲータ | エラーメッセージ、URL、関係するファイルを読み、次に確認することを声に出して説明する |
| 記録・説明係（3人の場合） | どの症状を見て、どこを直したかをメモし、修正後の確認結果を説明する |

進め方は次の通りです。

1. まず全員で、ブラウザに出ている症状やエラーを読む
2. ナビゲータが「どのファイルを見るか」「何を直すと考えたか」を説明する
3. ドライバが修正し、ブラウザで結果を確認する
4. 1つの故障を直したら役割を交代する
5. 自分たちで確認したあとに、解答例を開いて比べる

> [!IMPORTANT]
> すぐに解答例を開いて写すのではなく、先にチーム内で「今どこで止まっているか」「なぜこの修正をするのか」を言葉にしてください。
> 修正できたかどうかだけでなく、処理の流れを説明できることが今日のゴールです。
...
> - ドライバとナビゲータを交代しながら、原因と確認結果を言葉にする
