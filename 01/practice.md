# 第1週：練習 ── <ruby>scaffold<rt>スキャフォールド</rt></ruby>で思い出す
...
> [!TIP]
> **scaffold（スキャフォールド）とは？**
> `scaffold` は「スキャフォールド」と読みます。もともとは「足場」という意味です。Rails では、記事の一覧・作成・編集・削除を動かすためのひな形を、まとめて作ってくれる機能として使います。
...
- コードを書いたら、「なぜこう書くのか」を説明できるか
- ファイルを開いたら、「このファイルは何をしているか」を説明できるか
- エラーが出たら、「なぜエラーになったか」を説明できるか
...
scaffoldで動くものは作れた。だが、scaffoldが何をしているか説明できるだろうか。「動く」と「わかる」は違う。この授業では「わかる」の方に進む。

まずは今日、scaffoldが作ったものを自分の手で壊して、直して、仕組みを確かめるところから始めよう。[Stretch](stretch.md) に進んでください。
