...
準備が整いました。ここからは**課題①、課題②、課題③、課題④の順番に**、一つずつ作業を進めます。

---

## 課題①：【担当A】ヘッダーロゴと README の変更

担当A（代表者またはメンバーの1人目）が作業を行います。**担当A以外の人（B, C）は、Aの作業を見守りながら、自分が思っていることと違うことをしようとしていたら質問をしたり、正しい手順をアドバイスしてください。**
...
### Step 3：変更状態と差分を確認する
1. ファイルを保存したら、ブラウザのプレビュー画面を再読み込みし、ロゴの隣にチーム名が正しく表示されることを確認します。
2. どのファイルが変更されたか確認します。
   ```bash
   git status
   ```
...
3. 問題がなければ、コメント欄に「確認しました」「LGTM（Looks Good To Me：問題なさそうです）」などと入力します。
   ![](../images/LGTM.png)
...
担当B（グループの2人目）が作業を行います。**担当B以外の人（A, C）は、Bの作業を見守りながら、自分が思っていることと違うことをしようとしていたら質問をしたり、正しい手順をアドバイスしてください。**
...
### Step 3：変更状態と差分を確認する
1. ファイルを保存したら、ブラウザのプレビュー画面を再読み込みし、チーム名が正しく表示されることを確認します。
2. 状態を確認します。
   ```bash
   git status
   ```
...
担当C（グループの3人目。2人グループの場合は担当Aがもう一度行います）が作業を行います。**担当C以外の人（A, B）は、Cの作業を見守りながら、自分が思っていることと違うことをしようとしていたら質問をしたり、正しい手順をアドバイスしてください。**
...
### Step 3：変更状態と差分を確認する
1. ファイルを保存したら、ブラウザのプレビュー画面を再読み込みし、新規投稿フォームのボタンが「知識を送信する」に変わっていることを確認します。
2. 状態を確認します。
   ```bash
   git status
   ```
...
1. 準備4と同様に、ブラウザで `/invitations` のURLを開くか、GitHubの通知ベルから招待を **Accept（承諾）** します。
2. 承諾できたら、Codespace のターミナルに戻り、もう一度同じ push コマンドを実行します。
   ```bash
   git push origin test-permission
   ```
3. エラーが表示されず、`test-permission` ブランチがGitHubへ送信されたことを確認します。
...
そこで、グループ内で代表者を交代し、**最初から（準備1から）もう一度演習を行ってください。**

- **2周目**：代表者をメンバーBに変更して、BのGitHubアカウントでリポジトリを作成し、AとCを招待して課題①と④を実行します。
- **3周目**：代表者をメンバーCに変更して、CのGitHubアカウントでリポジトリを作成し、AとBを招待して課題②と④を実行します。
