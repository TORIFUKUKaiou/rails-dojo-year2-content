...
3. リポジトリの設定を次のように入力して作成します。
   ...
   - **Public / Private**: 必ず **Public** を選択してください。（Privateにすると、他のメンバーがアクセスできない場合があります）
4. リポジトリが作成されたら、そのページのURLをコピーして、Teamsや口頭などでグループのメンバー全員に伝えてください。

## 課題3：【代表者】メンバーをコラボレータ（共同開発者）に招待する
...
担当A（代表者またはメンバーの1人目）が作業を行います。**担当A以外の人（B, C）は、Aの作業が終わるまで待ちます。**
...
担当B（グループの2人目）が作業を行います。**担当B以外の人（A, C）は、Bの作業が終わるまで待ちます。**
...
担当C（グループの3人目。2人グループの場合は担当Aがもう一度行います）が作業を行います。**担当C以外は待ちます。**
...
## 課題8：【権限を外されたメンバー】トピックブランチを作成し、変更して push する
...
## 課題11：`git switch -c` と `git switch` の違いを説明する（考察問題・実行しない）
...
## 課題12：なぜマージの後に `git pull` を行うのか説明する（考察問題・実行しない）
...
おつかれさまでした。
実機での動作確認が終わったら、次の [Stretch](stretch.md)（コンフリクトの解決）へ進みましょう。
