...
```mermaid
flowchart TD
  A["同じ場所を同時に変更する"] --> B["先に一方が merge される"]
  B --> C["もう一方が push/PR すると GitHub 上で Conflict 発生"]
...
リポジトリは、**Practice で使用した代表者Aさんのリポジトリ**をそのまま使用し、全員がその Codespace に接続した状態で進めます。

> [!CAUTION]
> Rubyのコードを競合させるとアプリが起動しなくなる恐れがあるため、今回は安全な `README.md` を使用して競合を発生させます。

### 準備：手元の環境を最新にする（全員）

演習を始める前に、全員の手元の Codespace を最新の `main` 状態に揃えます。**Aさん、Bさん、Cさん全員が実行してください。**
...
担当Aと担当Bが同時に `README.md` の最終行を変更し、先にAがマージを完了させ、**Bがコンフリクトを解決する役割**を体験します。
...
※ 担当AとCは、Bの画面を監視し、手順に誤りがないかアドバイスをしてください。
...
6. **全員**: ターミナルで `main` ブランチに戻り、最新の変更を取り込みます。
   ...
   git pull
   ```
   全員のブラウザプレビュー（または `README.md`）に、AとB両方の文章が並んで表示されれば、シナリオ①は無事完了です。

---

### シナリオ②：【担当C】がコンフリクトを解決する

担当Bと担当Cが同時に `README.md` の最終行を変更し、先にBがマージを完了させ、**Cがコンフリクトを解決する役割**を体験します。
...
担当Cと担当Aが同時に `README.md` の最終行を変更し、先にCがマージを完了させ、**Aがコンフリクトを解決する役割**を体験します。
...
担当1（トピックブランチ名: `rename-header-link`）が作業を行います。他のメンバーは作業を監視し、アドバイスを行ってください。
