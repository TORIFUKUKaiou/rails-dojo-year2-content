...
- review と merge の役割を説明できる
- チームで同じリポジトリを扱うときの基本的な流れをイメージできる

> [!IMPORTANT]
> このオリエンテーションでは、操作手順を細かく扱いません。
> 実際のコマンド入力や GitHub 画面での操作は、この後の Practice で行います。
...
## review は責めるためではなく、よくするための確認

<ruby>review<rt>レビュー</rt></ruby> は、PR の変更内容をチームメンバーが確認することです。

review の目的は、間違い探しをして責めることではありません。
チームでコードを安全に育てることです。

review では、たとえば次のような視点で確認します。
...
review コメントは、相手を否定するためのものではありません。
「この変更をよりよくするにはどうすればよいか」をチームで相談するためのものです。

> [!NOTE]
> 後期のチーム開発では、PR と review を使って作業を進めます。
...
- 代表者だけが分かる状態を避ける
- branch、PR、review、merge の流れを何度も見る
- 役割が変わっても、同じ流れで作業できるようにする
- 後期のチーム開発で使う流れを、前期のうちに体験しておく

> [!IMPORTANT]
> Practice では、同時に別々の変更を進めるのではなく、1つずつ順番に進めます。
> まずはコンフリクトを起こさず、GitHub Flow の基本の流れを確実に練習します。
...
| review | PR の内容を読み、必要に応じてコメントすること |
| merge | 確認済みの変更を `main` に合流させること |
...
まずは、1つずつ順番に進めながら、branch、PR、review、merge の役割を確認していきましょう。
