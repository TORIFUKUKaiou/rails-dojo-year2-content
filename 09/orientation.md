...
たとえば、ファイルを保存する場所が必要になった場合、AWSのS3というサービスを使えば、数分でファイルの保存先を用意できます。

#### 初期費用をおさえて始められる（従量課金）

自分でサーバーを用意する場合、最初に機器を購入したり、設置場所を用意したりする必要があります。

クラウドでは、多くの場合、使った分に応じて料金が発生します。
このような料金の仕組みを、従量課金といいます。

最初から高価なサーバーを購入するのではなく、小さく始めて、必要に応じて大きくしていくことができます。
...
自分でサーバーを用意する場合、最初にどのくらいの性能が必要かを予想しておく必要があります。

しかし、最初から正確に予想することは簡単ではありません。
...
クラウドを使うと、インターネットを通じて、世界中の利用者にサービスを届けやすくなります。

たとえば、日本で作ったWebページでも、URLを知っている人であれば、別の地域からアクセスできます。

さらに、クラウドには世界各地にデータセンターがあります。
必要に応じて、利用者に近い場所からサービスを提供する構成も選べます。
...
S3は、**オブジェクトストレージ**と呼ばれる種類のストレージです。

パソコンのフォルダにファイルを保存する感覚と似ていますが、通常のファイルシステムとは仕組みが異なります。

通常のファイルシステムでは、ファイルの一部を書き換えるような使い方ができます。

一方、S3では、保存したデータをオブジェクトという単位で扱います。
そのため、オブジェクトの一部だけを書き換えるというより、変更後のファイルをもう一度アップロードして、オブジェクト全体を置き換えるイメージになります。

たとえば、`index.html` の一部を修正した場合でも、S3上では修正後の `index.html` をアップロードし直します。
...
AWSのリージョンは、北米、南米、ヨーロッパ、中東、アフリカ、アジアパシフィックなど、世界中に広がっています。

公式サイトでは、AWSのリージョンが世界中に配置されている様子を地図で確認できます。
...
Sandboxは、AWSの操作を練習するための一時的な環境です。
...
Sandboxは、起動してから180分で自動的に終了します。
Sandboxが終了すると、作成したリソースは削除されます。
