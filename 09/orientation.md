...
これまでWebアプリを動かすときは、主に自分のパソコンやCodespacesを使ってきました。

しかし、自分のパソコンで動いているだけでは、ほかの人がいつでもアクセスできるWebサービスにはなりません。

インターネット上でWebサイトやWebアプリを公開するには、次のようなものが必要です。

- ファイルを保存する場所
- プログラムを動かすサーバー
- データを保存するデータベース
- インターネットからの通信を受け付ける仕組み

クラウドでは、これらを自分で物理的に用意するのではなく、インターネット経由で必要な分だけ利用します。
