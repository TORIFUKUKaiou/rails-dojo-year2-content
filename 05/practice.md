...
この時点では、まだ `index` アクションや `index.html.erb` を作っていません。
`index` がない、またはテンプレートがないというエラーになります。
このエラーは、次に作るものを確認するためのものです。
...
サーバーが動いている状態で、ブラウザの `/articles` を再読み込みしてください。
