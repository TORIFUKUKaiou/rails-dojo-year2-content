...
> [!NOTE]
> このページは、授業冒頭に先生の実演を見ながら読む教材です。ここでは自分のファイルを変更したり、コマンドを実行したりしません。
...
実演で使うファイル：Railsアプリ内の `spec/total_spec.rb`
...
実演では、RSpecを導入済みのRailsアプリで、`Gemfile` があるフォルダから次のコマンドを実行します。
...
> [!IMPORTANT]
> 次は、失敗の表示を確認するために、先生が意図的に期待値を変更する実演です。
...
この実演では、正しい合計は300円です。先生が期待値を `300` に戻して保存し、同じコマンドを再実行すると、`1 example, 0 failures` に戻ります。
...
実演で使うファイル：Railsアプリ内の `spec/models/article_spec.rb`
