...
今日は、ファイルの変更を記録し、GitHubへ送信する一連の流れを学びます。

今日のゴールは次の4つです。

- Git と GitHub の役割を説明できる
- 変更を確認して commit（コミット）する流れを説明できる
- commit 履歴を確認できる
- commit した内容を GitHub へ push（プッシュ）する意味を説明できる
...
そこで今週は、書いたコードや修正したコードを安全に記録し、提出・共有するための強力な道具である **Git** と **GitHub** を学びます。
...
Git（ギット）は、このような問題を解決するための**バージョン管理システム（構成管理ツール）**です。
...
> 「意味のある変更の記録」にメッセージをつけて、アルバムの写真のように残しておくための道具です。
...
GitHub（ギットハブ）は、Gitで記録した履歴をインターネット上に置いておけるWebサービスです。

GitHubを使う理由は主に次の4つです。

- **バックアップ**: インターネット上にコードが保存されるため、PCが壊れても安心です
- **提出と共有**: 先生やチームメンバーに自分の成果を簡単に見せられます
- **チーム開発**: 後期で取り組む「チーム共同での開発」や、変更をレビューし合う「Pull Request（プルリクエスト）」がスムーズに行えます
- **Codespacesの土台**: Codespacesは、GitHub上にあるコード（リポジトリ）をもとにして、あなた専用の開発環境を瞬時に作ります
...
| 道具 | 役割 | たとえるなら？ | 動く場所 |
|---|---|---|---|
| **Git** | ファイルの変更履歴を記録するツール | カメラ（写真を撮って記録する） | 自分のPCやCodespacesの中（ローカル） |
| **GitHub** | Gitで記録したコードをネット上で共有・保管するWebサービス | クラウドのフォトアルバム（写真を共有・保管する） | インターネット上（リモート） |
...
Gitで変更を記録し、GitHubへ送信するまでには、いくつかのステップがあります。
...
| **2. ステージングエリア** | 次の「記録（コミット）」に**含める変更を選択して置く**場所です。「インデックス」とも呼ばれます。 |
...
Codespacesのターミナルを使って、実際に変更を記録・送信するコマンドを学びます。
...
変更したファイルがある場合、ターミナルに赤色や緑色でファイル名表示されます。
...
> [!TIP]
> コミット（記録）する前に、自分の意図した変更だけが入っているかを `git diff` で確認することは、プロの開発者も必ずやっているとても重要な習慣です。

### 3. `git add`（コミットに含める変更を選ぶ）

変更したファイルを、ステージングエリア（コミットする準備場所）に乗せます。
...
これによって、「このファイルの変更を次のコミットに含めるぞ」と指定したことになります。
...
ステージングエリアに置いた変更を、ローカルリポジトリに歴史（コミット）として正式に記録します。

```bash
git commit -m "READMEに自己紹介を追加"
```

`-m` は「メッセージ」の頭文字です。後ろのダブルクォーテーション `""` の中に、**「今回どんな変更をしたか」**が他の人（や未来の自分）に伝わる短い説明を書きます。

> [!WARNING]
> `-m` をつけずに `git commit` だけを実行すると、ターミナル上で見慣れない特殊なテキストエディタ（`vi` など）が起動してしまい、操作できなくなることがあります。
> 必ず `-m "メッセージ"` をつけて実行するようにしましょう。

### 5. `git log`（コミット履歴を見る）

これまでに記録したコミットの歴史を一覧で確認します。

```bash
git log
```

誰が・いつ・どんなメッセージでコミットしたかが上から新しい順に表示されます。
...
> [!IMPORTANT]
> **【超重要】`git log` 画面からの戻り方**
> `git log` を実行すると、ターミナルが履歴表示モードになり、入力ができなくなることがあります。
> その画面から元のターミナルに戻るには、**キーボードの `q`** を押してください。

### 6. `git push origin main`（GitHubへ送る）

手元のローカルリポジトリに記録したコミット履歴を、GitHub（リモートリポジトリ）へ送信します。

```bash
git push origin main
```

- `origin`（オリジン）: 送信先であるGitHubのリポジトリの場所を指す名前です。
- `main`（メイン）: コードを保存するメインの枝（ブランチ）の名前です。

手元で `git commit` をしただけでは、まだGitHubには変更が届いていません。
**最後に `git push` を行うことで、初めてGitHub上にコードがアップロードされ、バックアップや提出が完了します。**
...
これは、「今いるフォルダで、新しくGitでの歴史記録を開始する」というコマンドです。

> [!NOTE]
> 今回の演習やCodespacesを使った授業では、すでに最初からGitで管理されているリポジトリを使うことがほとんどです。
> すでにGitで管理されている場所でむやみに `git init` を実行すると、設定が壊れてしまう原因になるため、**今回の授業では実行しないようにしてください。**「新しくGitを始めるときに使うコマンドなんだな」と知っておくだけで十分です。

---

## 次回（第8週）の予告

今回は、メインの歴史（`main` ブランチ）に直接変更を記録して送る流れを学びます。

しかし、実際の開発や後期のチーム開発では、みんなが同じ場所を同時に書き換えると、コードが混ざって壊れてしまいます。
そのため、来週の第8週では、歴史を一時的に枝分かれさせて別々に作業する **「ブランチ（branch）」** や、それを合流させる **「マージ（merge）」**、そしてGitHub上でコードをレビューし合う **「Pull Request（プルリクエスト）」** の基本について学びます。
...
| `git diff` | 具体的にどこを書き換えたか、差分を確認する | `git add` する前、またはした直後 |
| `git add <ファイル名>` | 記録したい変更ファイルを選択する | 変更を確認したあと |
| `git commit -m "メッセージ"` | 選択した変更をメッセージ付きで記録する | addで選択し終わったあと |
| `git log` | これまでの記録（コミット）の歴史を見る | コミットが成功したか確認したいとき（戻るには `q`） |
| `git push origin main` | 手元のすべての記録をGitHubへ送信する | コミットが終わって、GitHubに送りたいとき |

それでは、実際にCodespacesを動かして、変更をGitHubに送信してみましょう！
